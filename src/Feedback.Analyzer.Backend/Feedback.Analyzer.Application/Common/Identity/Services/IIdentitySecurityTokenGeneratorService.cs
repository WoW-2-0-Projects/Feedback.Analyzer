...
public interface IIdentitySecurityTokenGeneratorService
{
    ...
    /// </summary>
    /// <param name="client">The user for whom the access token is generated.</param>
    /// <returns>An AccessToken object representing the generated access token.</returns>
    AccessToken GenerateAccessToken(Client client);
    ...
    RefreshToken GenerateRefreshToken(Client client, bool extendedExpiryTime = false);
    ...
    JwtSecurityToken GetToken(Client client, AccessToken accessToken);
    ...
    List<Claim> GetClaims(Client client, AccessToken accessToken);
}
