...
public class OrganizationDeleteByIdCommandHandler(
    ...
    ) : ICommandHandler<OrganizationDeleteByIdCommand, bool>
{
    public async Task<bool> Handle(OrganizationDeleteByIdCommand request, CancellationToken cancellationToken)
    {
        var result = await organizationService.DeleteByIdAsync(request.OrganizationId, cancellationToken: cancellationToken);
